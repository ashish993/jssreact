...
var find_app_name_1 = require("./find-app-name");
var secret_patch_1 = require("./secret-patch");
var userConfigFileName = 'scjssconfig.json';
 ...
 */
function getInteractiveData(initialData, allowInteraction, paramName, prompt, examplePrompt, validation, validationMessage, skipValidationIfNonInteractive) {
    if (skipValidationIfNonInteractive === void 0) { skipValidationIfNonInteractive = false; }
 ...
 */
function setup(interactive, outputFile, initialData, configName) {
    if (configName === void 0) { configName = 'sitecore'; }
    var getValidation = function (regexp) { return ((initialData === null || initialData === void 0 ? void 0 : initialData.skipValidation) ? undefined : regexp); };
    var config = {
        sitecore: {
            instancePath: '',
        },
    ...
    if (outputFile && fs_1.default.existsSync(outputFile)) {
        try {
            var existingFile = fs_1.default.readFileSync(outputFile, 'utf8');
    ...
    // DEPLOY URL/LS HOST
    var defaultDeployUrl = '/sitecore/api/jss/import';
    ...
    if (configObject.deploySecret) {
        var appConfig = glob_1.default
            .sync('./sitecore/config/*.config')
            .find(function (file) { return !file.match(/deploysecret/); });
        if (appConfig) {
            var appName = find_app_name_1.findAppNameInConfig(appConfig);
            if (appName) {
                var patchFile = path_1.default.resolve("./sitecore/config/" + appName + ".deploysecret.config");
